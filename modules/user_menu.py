...
from modules.pseudo_frontend import success, error, warning, menu, clear_terminal, user_message, contact_message
    ...
    def view_contacts(self):  
        while True:
            ...
            for i, contact in enumerate(contacts_list, start=1):
                options.append({'title': contact[1], 'number': i, 'function': lambda: False})
            ...
            if not options:
                ...
                try: menu(options, success_text='Contacts', error_text=False, input_message=False)
                except KeyboardInterrupt: break
    ...
    def manage_requests(self):
        ...
        while True:
            try:
                ...
                options = []
                for i, contact in enumerate(contacts_list, start=1):
                    options.append({'title': contact[1], 'number': i, 'function':interact_request, 'args': (contact, self.id)})
                ...
                if not options:
                    ...
                    title = menu(options, success_text=title, error_text='Invalid Request', input_message='Choose a request: ')
            except KeyboardInterrupt: break
...
def chat_option(user):
    while True:
        try:
            ...
            options = []
            for i, contact in enumerate(contacts_list, start=1):
                options.append({'title': contact[1], 'number': i, 'function': user.chat, 'args': (contact,)})
            if not options:
                    clear_terminal()
                    error('You have no contacts')
                    getpass('')   
            else:    
                menu(options, success_text='Chats', error_text='Invalid Chat', input_message='Choose a chat: ')
        except KeyboardInterrupt: break
...
def contact_option(user):
    ...
    options = [
        {'title': 'View Contacts', 'number': 1, 'function': user.view_contacts, 'message': 'Contacts'},
        {'title': 'Send Request', 'number': 2, 'function': user.send_request, 'message': 'Send a request'},
        {'title': 'Manage Requests', 'number': 3, 'function': user.manage_requests, 'message': 'Requests'}
    ]
    
    while True:
        try: menu(options, success_text='Contacts', error_text='Invalid Option')
        except KeyboardInterrupt:
            break     
...
def user_menu(id):
    ...
    user = User(id)
    options = [
        {'title': 'Chat', 'number': 1, 'function': chat_option, 'args': (user,)},
        {'title': 'Contacts', 'number': 2, 'function': contact_option, 'args': (user,)},
        {'title': 'Log out', 'number': 9, 'function': lambda: '__exit__'},
        {'title': 'Exit', 'number': 0, 'function': os._exit, 'args': (0,), 'message': 'Goodbye!'}
    ]
    while True:     
        try:
            login = menu(options, success_text=f'Welcome {user.username}', error_text='Invalid Option')
            if login == '__exit__':
                break
