...
from modules.pseudo_frontend import success, error, menu, clear_terminal, warning
...
def auth_menu():
    ...
    user = Auth()
    option_list = [
        {'title': 'Register', 'number': 1, 'function': user.register},
        {'title': 'Log in', 'number': 2, 'function': user.login},
        {'title': 'Exit', 'number': 0, 'function': os._exit, 'args': (0,), 'message': 'Goodbye!'}
    ]
    while True:
        try:
            id = menu(option_list, success_text='Welcome to Secure Chat', error_text='Invalid Option', print_option_title=True)
            if id: return id
